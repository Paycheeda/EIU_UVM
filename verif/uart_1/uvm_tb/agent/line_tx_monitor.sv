/*class line_tx_monitor extends uvm_monitor;
  ...
  // ---> FIXED: Changed from main_phase to run_phase <---
  ...
  task collect_data();
    ...
    // Force the parameters to match what we configured in the RTL (BKP Address 0)
    int local_baudrate   = 115200;
    int local_data_width = 8;
    bit local_parity_en  = 1'b0;
    ...
    forever begin
      ...
      pkt.parity_odd_even = 1'b0;

      clock_delay = clock_frequency / local_baudrate; 
      ...
      // Sample Parity (Skipped since parity is disabled)
