...
begin
	...
	begin
		case(state)
			IDLE:
			begin
				if(eth_frame_ready_pulse)
				begin
					eth_frame_ready_received <= 1;
				end
				...
				if((eth_frame_ready_received  || eth_frame_ready_pulse) && (eth_tx_start_received || eth_tx_start_pulse))
				begin
					eth_frame_ready_received <= 0;
					eth_tx_start_received <= 0;
					byte_counter <= 0;
