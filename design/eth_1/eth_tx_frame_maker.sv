...
module eth_tx_frame_maker(
		...
		output reg read_udp_payload_pulse,
		input udp_payload_byte_done_pulse,
...
localparam STATIC_STREAM_STATE        = 4'd1;
localparam UDP_CHECKSUM_DETECT_STATE  = 4'd2;
localparam UDP_CHECKSUM_STREAM_STATE  = 4'd3;
localparam UDP_PAYLOAD_ENABLE_STATE   = 4'd4;
localparam UDP_PAYLOAD_DETECT_STATE   = 4'd5;
localparam UDP_PAYLOAD_STREAM_STATE   = 4'd6;
localparam CRC_DONE_DETECT_STATE      = 4'd7;
localparam CRC_WRITE_STATE            = 4'd8;
...
assign static_stream_fire = (state == STATIC_STREAM_STATE) &&
                            !fifo_full;

assign udp_checksum_stream_fire = (state == UDP_CHECKSUM_STREAM_STATE) &&
                                  !fifo_full;

assign payload_stream_fire = (state == UDP_PAYLOAD_STREAM_STATE) &&
                             !fifo_full;

assign crc_write_fire = (state == CRC_WRITE_STATE) &&
                        !fifo_full;
...
begin
	...
	begin
		...
		if((byte_counter == UDP_CHECKSUM_BYTE_1) && (udp_payload_length == 16'd0))
		begin
			crc_last = 1'b1;
		end
	end

	else if(state == UDP_PAYLOAD_ENABLE_STATE)
	begin
		read_udp_payload_pulse = 1'b1;
	end

	else if(payload_stream_fire)
	begin
		data_out_fifo = udp_payload;
		fifo_wr_en    = 1'b1;

		data_out_crc  = udp_payload;
		crc_valid     = 1'b1;

		if(byte_counter == (udp_payload_length[10:0] - 11'd1))
		begin
			crc_last = 1'b1;
		end
		else
...
begin
	if(!rst_n)
	begin
		udp_checksum_read_ack      <= 1'b0;
		eth_frame_ready_pulse      <= 1'b0;
		crc_out                    <= 32'd0;
		byte_counter               <= 11'd0;
		preamble_word              <= 64'd0;
		dest_mac                   <= 48'd0;
			...
			begin
				...
				begin
					...
					begin
						...
						if(udp_payload_length == 16'd0)
						begin
							state <= CRC_DONE_DETECT_STATE;
						end
						else
						begin
							state <= UDP_PAYLOAD_ENABLE_STATE;
						end
					end
				end
				else
