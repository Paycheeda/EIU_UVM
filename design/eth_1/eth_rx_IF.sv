...
begin
    ...
    begin
        if(rx_dv_reg2 && byte_counter == 11'd0)
        begin
            rx_er_seen_in_frame <= 1'b0;
        end

        if(rx_dv_reg2 && rx_er_reg2)
        begin
            rx_er_seen_in_frame <= 1'b1;
        end
        
        if(rx_dv_reg2)
        begin
            if(rx_er_reg2 || rx_abort_active)
            begin
                ...
                begin
                    ...
                    if(byte_counter > 8)
                    begin
                        byte_counter_invalid <= byte_counter - 11'd8;
                    end
                    else
                    begin
                        byte_counter_invalid <= 11'd0;
                    end
                end
                else
                begin
            ...
            begin
                ...
                else if (byte_counter > 8 && byte_counter < 46)
                begin
                    rx_fifo_wr_en <= 1;
                    crc_first <= 0;
                    crc_valid <= 1;
                ...
                else if(byte_counter == 46)
                begin
                    udp_length[15:8] <= rxd_reg2;
                    rx_fifo_wr_en <= 1;
                    crc_valid <= 1;
                end
                else if(byte_counter == 47)
                begin
                ...
                else if(eth_packet_length_valid && byte_counter < eth_packet_length - 5)
                begin
                    rx_fifo_wr_en <= 1;
                    crc_first <= 0;
                    crc_valid <= 1;
        ...
        begin
            ...
            begin
                rx_abort_active <= 1'b0;
                if(byte_counter > 11'd8)
                begin
                    if(!eth_packet_length_valid || (byte_counter < (eth_packet_length - 11'd4)))
                    begin
                        early_rx_drop_detected <= 1'b1;
                        byte_counter_invalid   <= byte_counter - 11'd8;
                    end
...
begin
    ...
    begin
        ...
        case(state)
            IDLE:
            begin
                if(early_rx_drop_detected)
                begin
                    invalid_bytes <= byte_counter_invalid;
            ...
            begin
                ...
                begin
                    invalid_bytes <= eth_packet_length - 11'd12;
                    packet_received_corrupt_pulse <= 1;
                    crc_byte_count <= 0;
                    state <= ACQ_DONE_STATE;
                end
            ...
            begin
                if(crc_done_flag)
                begin
                    if(!rx_er_seen_in_frame && crc_received == crc_calc && preamble_word == PARAM_PREAMBLE)
                    begin
                        packet_received_corrupt_pulse <= 0;
                        state <= ACQ_DONE_STATE;
                    end
