	...
	begin
		...
		begin
			case(state_checksum)
				...
				begin
					if(ext_fifo_read_gap)
					begin
						ext_fifo_read_gap <= 1'b0;
					end
					else if((payload_byte_req_counter < udp_payload_length) && !ext_fifo_empty)
					begin
						ext_fifo_rd_en <= 1'b1;
						ext_fifo_read_gap <= 1'b1;
						payload_byte_req_counter <= payload_byte_req_counter + 1'b1;
					end
					...
					begin
						...
						begin
							...
							if((payload_byte_counter + 1'b1) == udp_payload_length)
							begin
								udp_checksum_accum <= udp_checksum_accum + {ext_fifo_data_out, 8'd0};
								second_byte_valid <= 1'b0;
								state_checksum <= FINAL_CHECKSUM_CALC_STATE;
							end
							else
							begin
								second_byte_valid <= 1'b1;
								state_checksum <= FIFO_NEXT_WORD_WAIT_STATE;
							end
						end
						else
